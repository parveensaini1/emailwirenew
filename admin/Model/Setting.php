...
class Setting extends AppModel {
    ...
    public function updateYaml() {
        ...
        $filePath = ROOT . DS . 'app' .DS. 'Config' . DS . 'settings.yml';
        $file = new File($filePath, true);
        $listYaml = Spyc::YAMLDump($list, 4, 60);
        $file->write($listYaml);
    }
    ...
    public function beforeSave($options = array()) { 
        ...
        if(!empty($this->data[$this->alias]['is_html'])){
            $this->data[$this->alias]['input_type']="html";
            $this->data[$this->alias]['value'] =htmlspecialchars($this->data[$this->alias]['value']);
        }else{
            $this->data[$this->alias]['input_type']="value";
            $this->data[$this->alias]['value'] =strip_tags($this->data[$this->alias]['value']);
        }
        return true;
    }
