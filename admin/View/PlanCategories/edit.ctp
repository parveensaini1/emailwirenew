                        ...
                        <div class="col-sm-3">
                            ...
                            <div class="card card-info">
                                ...
                                <div class="card-body" style="">
                                    ...
                                    $class=(!empty($this->data[$model]['is_country_allowed']))?"show":"hide";
                                    
                                    echo $this->Form->input('is_allowed_all_country', array('div' =>"form-group allcountrycheckbox div-status ml-2 $class", 'class' =>"custom_check", 'type' => 'checkbox', 'label' => 'Do want to select all country'));
                                    
                                    if(!empty($this->data[$model]['is_country_allowed'])){
                                        $class=(!empty($this->data[$model]['is_allowed_all_country']))?"hide":"show";
                                    }
                                    ...
                                    echo $this->Form->input('Country.Country', array('class' => 'form-control select2', 'onchange' => "get_multi_states('pccountryid','pcstateid')", 'multiple' => true, 'options' => $country_list, 'label' => false, 'data-select-all' => 'no', 'id' => "pccountryid", "div" => "countrydiv $class"));
                                    ...
                                    $stateClass=(!empty($this->data[$model]['is_state_allowed']))?"show":"hide";

                                    echo $this->Form->input('is_allowed_all_state', array('div' =>"form-group div-status allstatecheckbox ml-3 $stateClass", 'class' => 'custom_check', 'type' => 'checkbox', 'label' => 'Do want to select all state'));
                                    if(!empty($this->data[$model]['is_state_allowed'])){
                                        $stateClass=(!empty($this->data[$model]['is_allowed_all_state']))?"hide":"show";
                                    }
                                    ...
                                    echo $this->Form->input('State.State', array('class' => 'form-control select2', 'label' => false, 'type' => 'select', 'options' => $state_list, 'multiple' => true, 'div' =>"form-group statediv $stateClass", 'data-select-all' => 'no', 'id' => "pcstateid", 'onchange' => "get_multi_msas('pcstateid','pcmsaid')", 'multiple' => true));

                                    echo $this->Form->input('state_selected_ids', array('label' => false, 'type' => 'hidden', 'div' =>"form-group statediv $stateClass", 'id' => "filterstatevalues"));
                                    ...
                                    $msaClass=(!empty($this->data[$model]['is_msa_allowed']))?"show":"hide";

                                    echo $this->Form->input('is_allowed_all_msa', array('div' =>"form-group div-status ml-3 $msaClass allmsacheckbox", 'class' => 'custom_check', 'type' => 'checkbox', 'label' => 'Do want to select all MSA'));
                                    if(!empty($this->data[$model]['is_msa_allowed'])){
                                        $msaClass=(!empty($this->data[$model]['is_allowed_all_msa']))?"hide":"show";
                                    }

                                    echo '<a id="msa-all" style="font-size: 12px;" href="javascript:void(0)" class="'.$msaClass.' text-dark float-right text-right" onclick="selectAll(' . "'pcmsaid'" . ')">Select All dropdown msa</a>';
                                    echo $this->Form->input('Msa.Msa', array('label' => "City", 'class' => 'select2 form-control ', 'label' => false, 'options' => $mas_list, 'multiple' => true, 'id' => "pcmsaid", "div" => "msadiv $msaClass", 'empty' => "Select City"));
                                    echo $this->Form->input('msa_selected_ids', array('label' => false, 'type' => 'hidden', 'div' =>"form-group hide", 'id' => "filtermsavalues"));

                                    echo $this->Form->input('msa_ids', array('class' =>"form-control msaautodiv $msaClass", 'label' => false, 'type' => 'text', 'div' => 'form-group', 'id' => "searchmsa"));
                                    ...
                                    /*
                                        ...
                                        $class=(!empty($this->data[$model]['is_country_allowed']))?"show":"hide";
                                        echo '<a id="cntry-all" style="font-size: 12px;" href="javascript:void(0)" class="'.$class.' text-dark float-right text-right" onclick="selectAll('."'pccountryid'".')">Select All</a>';
                                        echo $this->Form->input('Country.Country', array('class' => 'form-control select2','onchange' => "get_multi_states('pccountryid','pcstateid')", 'multiple' => true, 'options' => $country_list, 'label' => false, 'id' => "pccountryid", "div" => "countrydiv $class"));

                                        echo $this->Form->input('is_state_allowed', array('div' => 'form-group div-status', 'class' => 'custom_check', 'type' => 'checkbox', 'label' => 'Is State Allowed'));
                                        
                                        $class=(!empty($this->data[$model]['is_state_allowed']))?"show":"hide";
                                        
                                        //echo '<a id="state-all" style="font-size: 12px;" href="javascript:void(0)" class="'.$class.' text-dark float-right text-right" onclick="selectAll('."'pcstateid'".')">Select All</a>';
                                        //echo $this->Form->input('State.State', array('class' => 'form-control select2', 'label' => false, 'type' => 'select', 'options' => $state_list, 'multiple' => true, 'div' =>"form-group statediv $class", 'id' => "pcstateid", 'multiple' => true, 'onchange' => "get_multi_msas('pcstateid','pcmsaid')"));
                                        
                                        
                                        $class=(!empty($this->data[$model]['is_msa_allowed']))?"show":"hide";

                                        echo $this->Form->input('is_msa_allowed', array('div' => 'form-group div-status', 'class' => 'custom_check', 'type' => 'checkbox', 'label' => 'Is MSA Allowed',));

                                        //echo '<a id="msa-all" style="font-size: 12px;" href="javascript:void(0)" class="'.$class.' text-dark float-right text-right" onclick="selectAll('."'pcmsaid'".')">Select All</a>';
                                       // echo $this->Form->input('Msa.Msa', array('label'=>"City",'class' => 'select2 form-control ', 'label' => false, 'options' => $mas_list, 'multiple' => true, 'id' => "pcmsaid", "div" => "msadiv $class"));
                            ...
                            <div class="card card-info">
                                ...
                                <div class="card-body" style="">
                                    ...
                                    if ($this->data[$model]['banner_image']) {
                                        $imgurl = 'files/company/press_image/' . $this->data[$model]['banner_path'] . '/' . $this->data[$model]['banner_image'];

                                        $removeFun = "removeUploadedImage('bannerimg'," . $this->data[$model]['id'] . ",'$model');";
                                        echo "<div id='remove-bannerimg'>" . $this->Html->image(SITEFRONTURL . $imgurl, array('id' => "imgprev-bannerimg", 'width' => "100%")) . "<a href='javascript:void(0)' class='btn btn-remove btn-danger' onclick=$removeFun>X</a></div>";
                                    }
                                    ?>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (obj.status == 'success') {
                    ...
                    message_box("Banner image removed successfully.", "Success", "success");
                    $("#" + inputId).attr("type", "file").val("").show();
                } else {
                    $("#" + inputId).after('<p class="error">' + obj.message + '</p>');
                    message_box(obj.message, "Failed", "failed");
...
$('#PlanCategoryIsStateAllowed').click(function() {
    if($(this).is(':checked')){
        ...
        $("#state-all").removeClass('hide');
    }else{
        $(".statediv").addClass('hide');
        $("#state-all").addClass('hide');
    }
});
