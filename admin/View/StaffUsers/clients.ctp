                        ...
                        <tbody>
                            ...
                            if (count($data_array) > 0) {
                                foreach ($data_array AS $index =>$data) { 
									...
									if ($data[$model]['staff_role_id'] == 3) {
                                     $actions .= ' ' . $this->Html->link(__('Company list'), array('controller' => $controller, 'action' => 'clientcompanies', $data[$model]['id'],$redirect), array('class' => 'btn btn-xs btn-primary'));
                                        
                                        $actions .= ' ' . $this->Html->link(__('Press Releases'), array('controller' => $controller, 'action' => 'press_release', $data[$model]['id']), array('class' => 'btn btn-xs btn-info'));

                                        $actions .= ' ' . $this->Html->link(__('Invoices'), array('controller' =>'Invoices', 'action' => '?s='.$data[$model]['email']), array('class' => 'btn btn-xs btn-primary'));

                                        $actions .= ' ' . $this->Html->link(__('Assign plan'), array('controller' =>'Plans', 'action' =>'assign_plans',$data[$model]['id'],'?redirect=clients'), array('class' => 'btn btn-xs btn-primary'));

                                        $actions .= ' ' . $this->Html->link(__('Activated plans'), array('controller' =>$controller, 'action' =>'activated_plans',$data[$model]['id']), array('class' => 'btn btn-xs btn-info'));
                                        
                                        $actions .= ' ' . $this->Html->link(__('Send Email'), array('controller' =>$controller, 'action' =>'email_to_client',$data[$model]['id']), array('class' => 'btn btn-xs btn-info'));
                                    ...
                                    $rows[] = array(
                                        ...
                                        __($data[$model]['email']), 
                                        $this->Custom->getEmailStatus($data[$model]['email_confirmed']),
