...
<?php /*?>
		<div class="podcast-assets assetbox">
			...
			if ($showTitle == 1) {
				echo '<div class="ew-title full">Poadcasts</div>';
			}
...
if (!empty($this->data['CompanyPresentation'])) { 
	$presentationHtml =""; ?>
	<div class="Company assetbox">
		<label class="ew-title full">Presentation</label>
		<?php
		foreach ($this->data['CompanyPresentation'] as $key => $presentation) {
...
<?php } 
if (!empty($this->data['CompanyEbook'])) { 
	$ebooktml =""; ?>
	<div class="Company assetbox">
		<label class="ew-title full">E-books</label>
		<?php
		foreach ($this->data['CompanyEbook'] as $key => $ebook) {
...
if (!empty($this->data['CompanyPodcast'])) { 
	$podtml =""; ?>
	<div class="Company assetbox">
		<label class="ew-title full">Podcast</label>
		<?php
		foreach ($this->data['CompanyPodcast'] as $key => $pod) {
		...
		foreach ($this->data['CompanyDocument'] as $docfiles_key => $docfiles_value) {
			if(!empty($docfiles_value['file_name'])){
				...
				if ($this->Custom->docType($docfiles_value['file_name']) == 'image') {
					$doc_image .= '<li class="col-sm-3"><a class="lightitem" data-sub-html="' . $docfiles_value['doc_caption'] . '" data-thumb="' . SITEURL . 'files/company/docfile/' . $docfiles_value['file_path'] . '/' . $docfiles_value['file_name'] . '" data-src="' . SITEURL . 'files/company/docfile/' . $docfiles_value['file_path'] . '/' . $docfiles_value['file_name'] . '"><img width="100%" title="" alt="" src="' . SITEURL . 'files/company/docfile/' . $docfiles_value['file_path'] . '/' . $docfiles_value['file_name']. '"></a><p class="text-center mt-1"><strong>'.$docfiles_value['doc_caption'].'</strong></p></li>';
...
<?php echo $this->element('newsroom_form_custom_pagination'); ?>
