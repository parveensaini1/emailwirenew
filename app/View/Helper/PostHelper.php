...
class PostHelper extends AppHelper
{
    ...
    {
        $noimageUrl = SITEURL . "img/ew_no_image.jpg";
        $noImgfileUrl = WWW_ROOT . 'img' . DS . 'ew_no_image.jpg';
        $noImgdest = WWW_ROOT . 'files' . DS . 'company' . DS . 'press_image' . DS . 'thumb' . DS . str_replace(".jpg", "", 'ew_no_image.jpg') . '-' . $width . 'x' . $height . '.jpg';
        ...
        if (!empty($imageArr)) {
            ...
            // $fileUrl = SITEURL . 'files/company/press_image/thumb/' . str_replace("." . $ext, "", $image_name) . '-' . $width . 'x' . $height . '.' . $ext;
            ...
            if (!file_exists($fileUrl)) {
                ...
                $dest = WWW_ROOT . 'files' . DS . 'company' . DS . 'press_image' . DS . 'thumb' . DS . str_replace("." . $ext, "", $image_name) . '-' . $width . 'x' . $height . '.' . $ext;
        ...
        //return $this->getLazyloadImage($imageUrl,['class' => $class, "width" => "100%", "alt" => $image_alt]);
        return $this->Html->image($imageUrl, array('class' => $class, "width" => "100%", "alt" => $image_alt));
        // return $this->Html->image($imageUrl, array('class' => $class, "width" =>$width."px", "height" =>$height."px", "alt" => $image_alt));
    }
    ...
    private function resizeImage($image_name, $imageUrl, $width = '', $height = "0", $prefix = "resize", $isOrignalname = "", $dest, $fileUrl)
    {
        ...
        if (extension_loaded('imagick')) {
            ...
            if (class_exists("Imagick")) {
                ...
                if ($width > 0 && $imageWidth > $width && $imageHeight >= $height) {
                    ...
                    if (!file_exists($dest)) {
                        ...
                        if ($this->crop_resize_image($fileUrl, $dest, 50, $width, $height)) {

                            $imageUrl = SITEURL . 'files/company/press_image/thumb/' . str_replace("." . $ext, "", $image_name) . '-' . $width . 'x' . $height . '.' . $ext;
                        }
                    } else {
            ...
            $image_data = array('width' => $width, 'height' => $height, 'file' => $imageUrl, 'output' => WWW_ROOT . 'files/company/press_image/thumb/', "prefix" => $prefix, "isOrignalname" => $isOrignalname);
    ...
    private function cropImage($image_name, $imageUrl, $width, $height, $x, $y, $prefix = "crop", $isOrignalname = "", $dest, $fileUrl)
    {
        ...
        if (extension_loaded('imagick')) {
            ...
            $this->Qimage->crop(array('w' => $width, 'h' => $height, 'x' => $x, 'y' => $y, 'file' => $imageUrl, 'output' => WWW_ROOT . 'files/company/press_image/thumb/', "prefix" => $prefix, "isOrignalname" => $isOrignalname));
    ...
    private function resizeAndCropImage($image_name, $imageUrl, $width, $height, $x = 0, $y = 10, $prefix = "reszcrp")
    {
        ...
        if (extension_loaded('imagick')) {
            ...
            $this->Qimage->resize(array('width' => $width, 'height' => $reheight, 'file' => $imageUrl, 'output' => WWW_ROOT . 'files/company/press_image/thumb/', "isOrignalname" => "true"));
            ...
            $file_path = WWW_ROOT . DS . 'files' . DS . 'company' . DS . 'press_image' . DS . 'thumb';
    ...
    public function getResizedImage($imageName = '', $width, $height, $prefix = "", $isOrignalname = "")
    {
        ...
        if (!empty($imageName)) {
            ...
            $output = SITEURL . 'files/company/press_image/' . $dir . '/' . $prefix . str_replace("." . $ext, "", $filebasename) . '-' . $width . 'x' . $height . '.' . $ext;

            if ((!empty($isOrignalname) && $isOrignalname == "true")) {

                $output = SITEURL . 'files/company/press_image/' . $dir . '/' . str_replace("." . $ext, "", $filebasename) . '.' . $ext;
            }
        }
    ...
    public function get_title($title = '', $slug = '')
    {
        ...
        return ($slug != '') ? "<a title='$title' class='ew-link-title' href='" . SITEURL . "release/" . $slug . "'>$title</a>" : $title;
    }
    ...
    public function getNewsroomLogo($logo_path = '', $logo = '', $slug = "")
    {
        ...
        if (!empty($slug)) {
            ...
            $logo = "<a href='" . SITEURL . 'newsroom/' . $slug . "'><img src='" . $imageUrl . "' width='50px' height='50px' /></a><meta itemprop='url' content='" . $imageUrl . "'>";
        }

        return $logo;
    }*/
    public function getNewsroomLogo($logo_path = '', $logo = '', $slug = "", $status = "")
    {

        // $obj = ClassRegistry::init('Company');
        // $data = $obj->find('first', array('conditions' => array('Company.slug' => $slug)));
        // $check_incomplete_newsroom = $data['Company'];  
        $slug = ($status == 1) ? $slug : "";
        ...
        if (empty($logo)) {
         ...
         $logo = $this->getLazyloadImage($imageUrl,['width'=>"50px", "height"=>'50px','class'=>'newsroomlogo']);
        // $logo='<img "width"="50px" height="50px" class="newsroomlogo" src="'.$imageUrl.'">';
        if (!empty($slug)) {
            $logo = "<a href='" . SITEURL . 'newsroom/' . $slug . "'>$logo</a>";
        }
    ...
    public function get_company($title = '', $slug = '', $status = "")
    {
        $slug = ($status == 1) ? $slug : "";
        ...
        if ($slug != '') {
            ...
            if ($check_incomplete_newsroom['status'] != 1) {
                ...
                return "<a class='ew-link-title' href='" . SITEURL . "newsroom/" . $slug . "'>$title</a>";
            }
        } else {
    ...
    //     return ($slug!='')?"<a class='ew-link-title' href='".SITEURL."newsroom/".$slug."'>$title</a>":$title;
    ...
    public function getSocialSharesHtml($title = '', $slug = '', $body = '', $image = '', $socialIcon, $sharer_url = '', $iconUrl = '')
    {


        $html = '';

        $html = '<li class="ew-' . $socialIcon . '"><a onclick="trackClickedEvents(' . "'" . $socialIcon . "'" . ',1)" target="_blank" id="prev-' . $socialIcon . '" rel="nofollow"  href="' . $sharer_url . ' "><img src="' . SITEURL . $iconUrl . '" /></a></li>';
    ...
    public function get_companyfile($docpath, $docfile)
    {
        ...
        return SITEURL . "files/company/docfile/" . $docpath . '/' . $docfile;
    }
    ...
    public function wordLimit($str, $slug = "", $charlimit = 35, $page = 'homepage')
    {
        ...
        if (count($pieces) > $charlimit) {
            ...
            $str = (!empty($slug)) ? implode(" ", array_splice($pieces, 0, (int)$charlimit)) . ' ...<a href="' . SITEURL . 'release/' . $slug . '">Read more</a>' : implode(" ", array_splice($pieces, 0, (int)$charlimit)) . ' ...';
        } elseif ($page == 'homepage') {

            $str = implode(" ", array_splice($pieces, 0, (int)$charlimit)) . ' ...<a href="' . SITEURL . 'release/' . $slug . '">Read more</a>';
        }

        return $str;
    }
    
    public function wordLimitForNewsroom($str, $newsroomUrl = "", $charlimit = 35, $page = 'homepage')
    {
        ...
        if (count($pieces) > $charlimit) {
            ...
            $str = (!empty($slug)) ? implode(" ", array_splice($pieces, 0, (int)$charlimit)) . ' ...<a href="' . $newsroomUrl . '">Read more</a>' : implode(" ", array_splice($pieces, 0, (int)$charlimit)) . ' ...';
        } elseif ($page == 'homepage') {

            $str = implode(" ", array_splice($pieces, 0, (int)$charlimit)) . ' ...<a href="' .$newsroomUrl . '">Read more</a>';
        }
    ...
    public function getNewsByCompany($cId = '')
    {
        ...
        $fields = ['PressRelease.id', 'PressRelease.country_id', 'PressRelease.company_id', 'PressRelease.title', 'PressRelease.slug', 'PressRelease.summary', 'PressRelease.release_date', 'PressRelease.language', 'Company.id', 'Company.name', 'Company.slug', 'Company.logo_path', 'Company.logo', 'Company.status'];
    ...
    public function summaryPrefix($sourceCity = '', $sourceState = '', $sourceCountry = '', $isSourceManually = '',$isOldRelease="")
    {
         
        if(!empty($isOldRelease)&&$isOldRelease=="1"){
            return "";
        }
        ...
        return '<span class="cntnt-prfx">' . $sourceName . "--(<a style='text-decoration:none;'target='_blank' rel='nofollow' href='" . SITEURL . "' title='" . $site_name . ".com'>" . $site_name . "</a>)--</span>";
    }
    ...
    public function getLazyloadImage($src,$attributes=[],$url=null,$hrefAttributes=[]){
        $defaultImageUrl="data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=";
        $class="lazyload";
        $attributesHtml="";
        if(!empty($attributes)){
            foreach($attributes as $attr => $attrValue ){
                if($attr=="class"){
                    $attributesHtml .="$attr='$attrValue $class' ";
                }else{
                    $attributesHtml .="$attr='$attrValue' ";
                }
        ...
        $hrefAttributesHtml="";
        if(!empty($attributes)){
            foreach($hrefAttributes as $attr => $hrefAttrValue ){
                $hrefAttributesHtml .="$attr='$hrefAttrValue' ";
                
            }
        } 

        if($src){
            // $image='<img '.$attributesHtml.' data-src="'.$src.'"> <meta itemprop="url" content="' . $src . '">';

            $image='<img '.$attributesHtml.' src="'.$src.'"> <meta itemprop="url" content="' . $src . '">';
            if(!empty($url)){
                '<a href="'.$url.' '.$hrefAttributesHtml.' ">'.$image.'</a>';
            }
        }else{
            $image='<img '.$attributesHtml.' src="'.$defaultImageUrl.'">';
        }
        return  $image;
    }
}
