...
class PostHelper extends AppHelper {
    ...
    public function getPrSingleImage($imageArr='',$isCropOrResize="",$width="",$height="0",$x="0",$y="0",$is_desc='',$class='press-image'){
        ...
        if(!empty($imageArr)){
            ...
            $class .=' '.'primg-'.$imageArr[0]['id'];

            $image_path=$imageArr[0]['image_path'];

            $image_name=$imageArr[0]['image_name'];

            $image_alt=(isset($imageArr[0]['image_text']))?$imageArr[0]['image_text']:"";

            $describe_image=(isset($imageArr[0]['describe_image']))?$imageArr[0]['describe_image']:"";
    ...
    public function get_title($title='',$slug=''){
        ...
        return ($slug!='')?"<a class='ew-link-title' href='".SITEURL."release/".$slug."'>$title</a>":$title;
    ...
    public function sharelinks($title='',$slug='',$body='',$image=''){
        ...
        if(!empty($socials)){
            ...
            foreach ($socials as $social_values) {
                ...
                $get_html = $this->getSocialSharesHtml($title,$slug,$body,$image,$socialIcon,$sharer_url);
            ...
            $url = SITEURL.'rss/release.rss?s='.$slug;

            $html .= $this->getSocialSharesHtml($title,$slug,$body,$image,'rss',$url);
    ...
    public function getSocialSharesHtml($title='',$gslug='',$body='',$image='',$socialIcon,$sharer_url = ''){

            $slug=$gslug;

            $html=''; 

            $html ="<li class='ew-".$socialIcon."'><a target='_blank' id='prev-".$socialIcon."' rel='nofollow'  href='".$sharer_url."'></a></li>";
    ...
    public function getNewsByCompany($cId=''){ 
        ...
        $fields=['PressRelease.id','PressRelease.country_id','PressRelease.company_id','PressRelease.title','PressRelease.slug','PressRelease.summary','PressRelease.release_date','Company.name','Company.slug','Company.logo_path','Company.logo'];
