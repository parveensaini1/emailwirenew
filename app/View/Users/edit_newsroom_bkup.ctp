...
<div class="full ew-account-page margin-bottom20">
        ...
        <?php }else{?>
            <div class="col-lg-12"><div class="ew-title full">Edit Newsroom</div></div>
        <?php } ?>
        ...
        <?php if(!$this->Session->read('Auth.User.id')){ ?>
            ...
            <div class="row">
                ...
                <!-- media detail -->
                <?php
                    echo $this->Form->input('Company.id', array("type" => 'hidden',"value"=>$company_id));
                ?>
                ...
                <div class="col-sm-6 form-group">
                    ...
                    <?php
                    echo $this->Form->input('Company.name', array("type" => 'text','readonly','maxlength'=>"100",'class'=>'form-control ',"required"=>"required","id"=>"company_name",'onchange'=>"search_company(); setCookie('CakeCookie[nr_company_name]',this.value)",'data-msg'=>"Please enter company name.")); //"onkeypress"=>"search_company();",
                    ?>
                ...
                <div class="col-lg-12 form-group">
                    ...
                    <?php
                    echo $this->Form->input('Company.hear_about_us', array("type" => 'textarea','class'=>'form-control ',"required"=>false,"col"=>"30","row"=>"30"));
                    ?>
                </div> 
                ...
                <div class="col-lg-12">
                    <div class="form-group ew-company-file">
                        ...
                        <?php
                        if(isset($remove_doc) && !empty($remove_doc)){
                            foreach ($remove_doc as $remove_doc_key => $remove_doc_value) {
                                echo '<input type="hidden" name="data[Company][remove_doc][]" value="'.$remove_doc_value.'">';
                            }
                        }
                        if(isset($nr_docfiles) && !empty($nr_docfiles[0]['name']) ){
                            foreach ($nr_docfiles as $doc_key => $doc_values) {
                                ...
                                echo $this->Form->input('Company.docfilescount.', array("type" => 'hidden','class'=>'form-control docfile','value'=>$doc_values['name'],'id'=>'docfile'.$plus_doc_key,'file_count'=>$plus_doc_key,'doc_id'=>$doc_values['id'],'required'=>false));
                            }
                        ?>
...
$('body').on('click','#rbtn',function(){
    ...
    minus_fnum = (fnum-1);
    var doc_id = parseInt($('#docfile'+fnum).attr('doc_id'));
    $('#docfile'+fnum).remove();
    $('#docfile-error'+fnum).remove();
    $('#filename'+fnum).remove();
    $('.docfile-btn').before('<input type="hidden" name="data[Company][remove_doc][]" value="'+doc_id+'">');
    if(minus_fnum > 0){
