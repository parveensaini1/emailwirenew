	...
	public function commonProcess($modelName = null, array $options = array()) {
		...
		if (!empty($this->controller->request->data)) {
			...
			if ($valid) {
				...
				$hashed 	= md5($this->controller->request['controller'] . '_'  . $this->controller->request['action']);
				$this->controller->Session->write('enc.'. $hashed, $searchParams);
				 
				$actions = array_diff_key($params, $searchParams);
				$actions['enc'] = $hashed; 
				
				$this->controller->redirect($actions);
			} else {
				...
				if(isset($this->controller->passedArgs['enc'])){
					 if($this->controller->Session->check('enc.'. $this->controller->passedArgs['enc'])){
						$this->controller->passedArgs =  $this->controller->Session->read('enc.'. $this->controller->passedArgs['enc']);
					 }
					//$decrypted = Security::decrypt($encrypted, 'wt1U5MACWJFTXGenFoZoiLwQGrLgdbHA');
					//$decrypted = base64_decode($encrypted,true);
					//$this->controller->passedArgs = unserialize( $decrypted);
						
					//$this->controller->passedArgs = 	unserialize(base64_decode($encrypted));
				}
