...
<?php require_once('../helpers/Dbconn.php');?>
<?php
...
$app = isset($_POST['app']) && is_numeric($_POST['app']);
$listID = isset($_POST['list_id']) && is_numeric($_POST['list_id']);
...
if(!file_exists($csvfile)) {
	...
	echo '<script type="text/javascript">window.location="'.addslashes(SITEADMINURL."/sendy/import_subscriber_csv/".$listID."?e=3").'";</script>';
	exit;
}
...
foreach ($csv->data as $key => $line){	
	...
	if($cron){
		if(!file_exists("../../uploads/csvs")){
			...
			echo '<script type="text/javascript">window.location="'.addslashes(SITEADMINURL."/sendy/import_subscriber_csv/".$listID."?e=4").'";</script>';
			exit;
			}else{
				//chmod uploaded file
		...
		if($columns != $custom_fields_count+2){
			...
			echo '<script type="text/javascript">window.location="'.addslashes(SITEADMINURL."/sendy/import_subscriber_csv/".$listID."?e=1").'";</script>';
			exit;
		}else{
			...
			if(move_uploaded_file($csvfile, "../../uploads/csvs/".$userID.'-'.$listID.'.csv')){
				...
				// header("Location: ".get_app_info('path').'/list?i='.$app); 
				 echo '<script type="text/javascript">window.location="'.addslashes(SITEADMINURL."/sendy").'";</script>';
					exit;
			}
	...
	if (mysqli_num_rows($r) > 0) //if so, update subscriber
	{
		...
		skipped_emails(trim($linearray[1]), 'Exists',$listID,$app);
	}
	else
	{			
		//Get the list of custom fields for this list
		...
		{			
			...
			{			
				...
				if($columns==1)
				{
					if ($validator->check_email_address(trim($linearray[0]))){
						...
						$query = 'INSERT INTO '.$databasetable.' (userID, email, list, timestamp, gdpr,staff_user_id) values('.$userID.', "'.trim($linearray[0]).'", '.$listID.', '.$time.', '.$gdpr_tag.','.$userID.')';
						mysqli_query($mysqli, $query);
						$inserted_id = mysqli_insert_id($mysqli);
					}
					else skipped_emails(trim($linearray[0]), 'Malformed',$listID,$app);
				}
				...
				else if($columns==2)
				{
					if ($validator->check_email_address(trim($linearray[1]))){
						...
						$query = 'INSERT INTO '.$databasetable.' (userID, name, email, list, timestamp, gdpr,staff_user_id) values('.$userID.', "'.strip_tags($linearray[0]).'", "'.trim($linearray[1]).'", '.$listID.', '.$time.', '.$gdpr_tag.','.$userID.')';
						mysqli_query($mysqli, $query);
						$inserted_id = mysqli_insert_id($mysqli);
					}
					else skipped_emails(trim($linearray[1]), 'Malformed',$listID,$app);
				}
			...
			{				
				...
				skipped_emails(trim($linearray[1]), 'Suppressed',$listID,$app);
			}
		}
		else
		{
			skipped_emails(trim($linearray[1]), 'Bounced',$listID,$app);
		}
	}
}
...
//--------------------------------------------------------------//
function skipped_emails($email, $reason,$listID,$app)
//--------------------------------------------------------------//
{
	global $mysqli;
...
echo '<script type="text/javascript">window.location="'.addslashes("/admin/sendy/subscribers/".$listID).'";</script>';
