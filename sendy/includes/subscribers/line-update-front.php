...
if($line=='')
{
	...
	echo '<script type="text/javascript">window.location="'.addslashes(SITEADMINURL."/sendy/add_subscriber/".$listID."?e=2").'";</script>';
	exit;
}
...
for($i=0;$i<count($line_array);$i++)
{
	...
	if (mysqli_num_rows($r) > 0) //if so, update subscriber
	{
		...
		skipped_emails($email, 'Exists',$app,$listID);
	}
	else
	{
		...
		if (mysqli_num_rows($r) == 0)
		{
			...
			if (mysqli_num_rows($r2) == 0)
			{
				...
				{
					$email = filter_var($email,FILTER_SANITIZE_EMAIL);
					$q = 'INSERT INTO subscribers (userID, name, email, list, timestamp, gdpr, staff_user_id) values('.$userID.', "'.$name.'", "'.trim($email).'", '.$listID.', '.$time.', '.$gdpr_tag.','.$userID.')';
					mysqli_query($mysqli, $q);
				}
				else skipped_emails(trim($email), 'Malformed',$app,$listID);
			}
			else
			{
				...
				skipped_emails(trim($email), 'Suppressed',$app,$listID);
			}
		}
		else skipped_emails(trim($email), 'Bounced',$app,$listID);
	}
}

//--------------------------------------------------------------//
function skipped_emails($email, $reason,$app,$listID)
//--------------------------------------------------------------//
{
	global $mysqli;
